...
void GBNManager::sendAck(uint8_t  ack_num, const sockaddr_in &targetAddr) {
    ...
    info << "[" << m_roleName << " RECV] Sent ACK: " << ack_num << endl;
}
...
void GBNManager::handleAck(uint8_t  ack_num) {
    ...
    if (count_acked > 0 && count_acked <= WINDOW_SIZE) {
        ...
        info << "[" << m_roleName << " SEND] Received valid ACK: " << ack_num
             << ". Base moved from " << old_base << " to " << m_senderState.base
             << endl;
        ...
        warn << "[" << m_roleName
             << " SEND] Received duplicate or invalid ACK: " << ack_num
             << ". Ignored." << endl;
    }
}
...
void GBNManager::handleData(const Packet &p, const sockaddr_in &senderAddr) {
    if (p.seq_num == m_receiverState.expected_seq_num) {
        // 收到期望的包
        info << "[" << m_roleName
             << " RECV] >>> Received DATA (Seq: " << p.seq_num << ")." << endl;
        ...
        warn << "[" << m_roleName
             ...
             << m_receiverState.expected_seq_num << ", Got: " << p.seq_num
             << ". Discarding." << endl;
...
bool GBNManager::sendData(const char *buffer, int len) {
    if (isWindowFull()) {
        warn << "[" << m_roleName << " SEND] Window full. Cannot send Seq "
             << m_senderState.next_seq_num << endl;
        return false;
    }
    ...
    info << "[" << m_roleName << " SEND] Sent DATA. Seq: " << p.seq_num << endl;
    ...
    if (GetTickCount() - m_senderState.last_send_time >= TIMEOUT) {
        ...
        for (uint16_t i = 0; i < count; ++i) {
            ...
            sendto(m_sock, (const char *)&p, sizeof(p), 0,
                   (const SOCKADDR *)&m_targetAddr, sizeof(m_targetAddr));
            info << "[" << m_roleName
                 << " SEND] Retransmit DATA. Seq: " << p.seq_num << endl;
        }
