...
#include "GBNmanager.h" // 依赖 GBNManager 类的定义
#include <winsock2.h>
#include <ws2tcpip.h>
...
// 内部 I/O 处理器
void _processNetworkIO(GBNManager &gbnManager, DWORD waitTimeMs = 10) {
    timeval tv;
    tv.tv_sec = 0;
...
std::vector<uint8_t> _read_n_bytes(GBNManager &gbnManager,
...
size_t sendData(GBNManager &gbnManager, const std::vector<uint8_t> &data) {
    std::string roleName = gbnManager.getRoleName();
    ...
    for (size_t offset = 0; offset < full_message_size;
        ...
        while (!chunk_sent) {
            _processNetworkIO(gbnManager);

            const char *buffer = (const char *)(full_message.data() + offset);

            if (gbnManager.sendData(buffer, chunk_size)) {
...
std::vector<uint8_t> recvData(GBNManager &gbnManager) {
    std::string roleName = gbnManager.getRoleName();
    ...
    std::vector<uint8_t> length_bytes =
        _read_n_bytes(gbnManager, LENGTH_PREFIX_SIZE);
    ...
    std::vector<uint8_t> payload = _read_n_bytes(gbnManager, payload_len);
