...
// 接收主机 MAC 地址 00:0c:29:05:82:fe
const unsigned char RECEIVER_MAC[6] = {0x00, 0x0c, 0x29, 0x05, 0x82, 0xfe};
...
void print_final_info(const unsigned char* buffer, ssize_t len) {
    ...
    std::cout << "\n=======================================================" << std::endl;
    std::cout << "[" << get_current_time() << " LOG: RECEIVER] 成功接收最终 IP 数据报 (" << len << " 字节):" << std::endl;
    std::cout << "  [L2] 源MAC地址: " << mac_to_str(eh->ether_shost) << " (来自路由器)" << std::endl;
    std::cout << "  [L2] 目的MAC地址: " << mac_to_str(eh->ether_dhost) << std::endl;
    std::cout << "  [L3] 源IP地址: " << src_ip << std::endl;
    std::cout << "  [L3] 目的IP地址: " << dst_ip << std::endl;
    std::cout << "  [L3] TTL: " << (int)iph->ttl << std::endl;
    std::cout << "  [L3] IP Checksum: 0x" << std::hex << ntohs(iph->check) << std::dec << " (应为有效)" << std::endl;
    
    if (iph->protocol == IPPROTO_UDP) {
         size_t ip_hdr_len = iph->ihl * 4;
         const struct udphdr *udph = (const struct udphdr *)(buffer + sizeof(struct ether_header) + ip_hdr_len);
         std::cout << "  [L4] UDP Src Port: " << ntohs(udph->source) << std::endl;
         std::cout << "  [L4] UDP Dst Port: " << ntohs(udph->dest) << std::endl;
         
         const char* payload = (const char*)(buffer + sizeof(struct ether_header) + ip_hdr_len + sizeof(struct udphdr));
         size_t payload_len = len - (sizeof(struct ether_header) + ip_hdr_len + sizeof(struct udphdr));
         std::cout << "  [DATA] 载荷内容: " << std::string(payload, payload_len) << std::endl;
    }
    std::cout << "=======================================================" << std::endl;
}
...
int main() {
    ...
    std::cout << "[" << get_current_time() << " LOG: RECEIVER] 目的主机启动，监听接口: " << INTERFACE_NAME << std::endl;
    ...
    while (1) {
        ...
        // 将期望的源 IP 字符串转为 in_addr
        struct in_addr expected_addr;
        if (inet_pton(AF_INET, SRC_IP_STR, &expected_addr) != 1) {
            std::cerr << "[" << get_current_time() << " LOG: RECEIVER] 无效的 SRC_IP_STR: " << SRC_IP_STR << std::endl;
            continue;
        }

        // 比较网络字节序的地址
        if (pkt_addr.s_addr != expected_addr.s_addr) {
            continue;
        }
